				...
				if(upsert.type === 'notify') {
					for(const msg of upsert.messages) {
						if(!msg.key.fromMe && doReplies) {
							...
							if(msg.message?.conversation?.startsWith('calc') || msg.message?.conversation?.startsWith('hitung')) {
								...
								if(expression) {
									...
									try {
										...
										await sendMessageWTyping({ text: 'Invalid math expression' }, msg.key.remoteJid!)
									}
								}
							}
						}
